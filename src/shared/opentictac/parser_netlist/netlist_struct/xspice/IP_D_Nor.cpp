#include "IP_D_Nor.hpp"

TICTAC_SPTR<IP_D_BaseElement>
    IP_D_Nor::promote_clone(std::string prefix)
{
  ...
  return create_d_nor(prefix + this->name_space(), this->model_name(), ins, out);
}

void
    IP_D_Nor::exchange_digital_in(const IP_NetworkNode* old_node,
...
std::tuple<std::string, std::string, std::string>
    IP_D_Nor::get_channel_info(IP_NetworkNode* nn)
{
                 ...
                 std::string out)
{
  ...
  auto d_nor = std::make_shared<IP_D_Nor>(IP_D_Nor(name, model_name));

  d_nor->ins(port_ins);
  for (auto& port_in : port_ins) port_in->digital_ins(d_nor);
  d_nor->out(port_out);
  port_out->digital_outs(d_nor);
  return d_nor;
}
