...
#ifndef TICTAC_IP_D_XNOR_HPP
#define TICTAC_IP_D_XNOR_HPP

#include "IP_D_BaseElement.hpp"
#include "IP_NetworkNode.hpp"
#include "sim_utils.hpp"

class IP_D_Xnor : public IP_D_BaseElement
{
 public:
  IP_D_Xnor(std::string name_space, std::string model_name)
  ...
  IP_D_Xnor& ins(const std::vector<TICTAC_SPTR<IP_NetworkNode>>& ins)
  {
    ins_ = ins;
    return *this;
  };

  IP_D_Xnor& in(int i, TICTAC_SPTR<IP_NetworkNode> network_node)
  {
    ins_[i] = network_node;
    return *this;
  };
  ...
  IP_D_Xnor& out(TICTAC_SPTR<IP_NetworkNode> network_node)
  {
    out_ = network_node;
    return *this;
  };
  ...
  {
    SIM_PRINT_L5(verb(), "We are in the D_Xnor!!!");
