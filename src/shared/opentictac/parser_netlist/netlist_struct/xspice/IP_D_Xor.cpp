#include "IP_D_Xor.hpp"

TICTAC_SPTR<IP_D_BaseElement>
    IP_D_Xor::promote_clone(std::string prefix)
{
  ...
  return create_d_xor(prefix + this->name_space(), this->model_name(), ins, out);
}

void
    IP_D_Xor::exchange_digital_in(const IP_NetworkNode* old_node,
...
std::tuple<std::string, std::string, std::string>
    IP_D_Xor::get_channel_info(IP_NetworkNode* nn)
{
                 ...
                 std::string out)
{
  ...
  auto d_xor = std::make_shared<IP_D_Xor>(IP_D_Xor(name, model_name));

  d_xor->ins(port_ins);
  for (auto& port_in : port_ins) port_in->digital_ins(d_xor);
  d_xor->out(port_out);
  port_out->digital_outs(d_xor);
  return d_xor;
}
