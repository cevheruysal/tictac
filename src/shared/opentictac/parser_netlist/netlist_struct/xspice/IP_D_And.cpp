#include "IP_D_And.hpp"

TICTAC_SPTR<IP_D_BaseElement>
    IP_D_And::promote_clone(std::string prefix)
{
  ...
  return create_d_and(prefix + this->name_space(), this->model_name(), ins, out);
}

void
    IP_D_And::exchange_digital_in(const IP_NetworkNode* old_node,
...
std::tuple<std::string, std::string, std::string>
    IP_D_And::get_channel_info(IP_NetworkNode* nn)
{
                 ...
                 std::string out)
{
  ...
  auto d_and = std::make_shared<IP_D_And>(IP_D_And(name, model_name));

  d_and->ins(port_ins);
  for (auto& port_in : port_ins) port_in->digital_ins(d_and);
  d_and->out(port_out);
  port_out->digital_outs(d_and);
  return d_and;
}
