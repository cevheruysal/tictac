#include "IP_D_Nand.hpp"

TICTAC_SPTR<IP_D_BaseElement>
    IP_D_Nand::promote_clone(std::string prefix)
{
  ...
  return create_d_nand(prefix + this->name_space(), this->model_name(), ins, out);
}

void
    IP_D_Nand::exchange_digital_in(const IP_NetworkNode* old_node,
...
std::tuple<std::string, std::string, std::string>
    IP_D_Nand::get_channel_info(IP_NetworkNode* nn)
{
                  ...
                  std::string out)
{
  ...
  auto d_nand = std::make_shared<IP_D_Nand>(IP_D_Nand(name, model_name));

  d_nand->ins(port_ins);
  for (auto& port_in : port_ins) port_in->digital_ins(d_nand);
  d_nand->out(port_out);
  port_out->digital_outs(d_nand);
  return d_nand;
}
