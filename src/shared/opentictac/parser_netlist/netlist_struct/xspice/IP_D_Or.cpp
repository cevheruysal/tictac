...
TICTAC_SPTR<IP_D_BaseElement>
    IP_D_Or::promote_clone(std::string prefix)
{
  ...
  return create_d_or(prefix + this->name_space(), this->model_name(), ins, out);
}

void
    IP_D_Or::exchange_digital_in(const IP_NetworkNode* old_node,
...
std::tuple<std::string, std::string, std::string>
    IP_D_Or::get_channel_info(IP_NetworkNode* nn)
{
                ...
                std::string out)
{
  ...
  auto d_or = std::make_shared<IP_D_Or>(IP_D_Or(name, model_name));

  d_or->ins(port_ins);
  for (auto& port_in : port_ins) port_in->digital_ins(d_or);
  d_or->out(port_out);
  port_out->digital_outs(d_or);
  return d_or;
}
